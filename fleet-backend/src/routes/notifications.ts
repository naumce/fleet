...
notificationsRouter.get("/driver/notifications", async (req, res) => {
  const notifications = await prisma.notification.findMany({
    where: { driverId: req.auth!.driverId }, orderBy: { createdAt: "desc" } });
  res.json(notifications);
});

notificationsRouter.post("/driver/notifications/read-all", async (req, res) => {
  const result = await prisma.notification.updateMany({
    where: { driverId: req.auth!.driverId, readAt: null }, data: { readAt: new Date() } });
  res.json({ updated: result.count });
});

notificationsRouter.put("/notifications/:id/read", async (req, res) => {
...
notificationsRouter.get("/driver/notification-preferences", async (_req, res) => {
  res.json({
    tripUpdates: true,
    messages: true,
